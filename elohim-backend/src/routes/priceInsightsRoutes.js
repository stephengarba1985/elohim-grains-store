...
const ensurePriceAlerts = () => pool.query(`CREATE TABLE IF NOT EXISTS product_price_follows (
  ...
  baseline_price DECIMAL(12,2) NOT NULL, threshold_percent DECIMAL(5,2) NOT NULL DEFAULT 3, alert_type VARCHAR(30) NOT NULL DEFAULT 'change', target_price DECIMAL(12,2), created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, PRIMARY KEY(user_id,product_id)
)`).then(() => pool.query("ALTER TABLE product_price_follows ADD COLUMN IF NOT EXISTS alert_type VARCHAR(30) NOT NULL DEFAULT 'change', ADD COLUMN IF NOT EXISTS target_price DECIMAL(12,2)"));

router.post("/follow", verifyToken, async (req,res) => {
  try { await ensurePriceAlerts(); const { product_id, threshold_percent, alert_type, target_price }=req.body; const product=await pool.query("SELECT id,price FROM products WHERE id=$1",[product_id]); if(!product.rows[0])return res.status(404).json({error:"Product not found"}); const threshold=Math.max(1,Number(threshold_percent)||3); const type=alert_type === "below_price" ? "below_price" : "change"; const target=Number(target_price); if(type === "below_price" && (!Number.isFinite(target) || target <= 0)) return res.status(400).json({error:"Enter a valid target price"}); const result=await pool.query(`INSERT INTO product_price_follows (user_id,product_id,baseline_price,threshold_percent,alert_type,target_price) VALUES ($1,$2,$3,$4,$5,$6) ON CONFLICT (user_id,product_id) DO UPDATE SET baseline_price=EXCLUDED.baseline_price,threshold_percent=EXCLUDED.threshold_percent,alert_type=EXCLUDED.alert_type,target_price=EXCLUDED.target_price,created_at=CURRENT_TIMESTAMP RETURNING *`,[req.user.id,product_id,product.rows[0].price,threshold,type,type === "below_price" ? target : null]);res.status(201).json(result.rows[0]); }
...
router.get("/alerts/me", verifyToken, async (req,res) => { try { await ensurePriceAlerts();const result=await pool.query(`SELECT f.*,p.name,p.price,ROUND(((p.price-f.baseline_price)/NULLIF(f.baseline_price,0))*100,2) AS change_percent FROM product_price_follows f JOIN products p ON p.id=f.product_id WHERE f.user_id=$1 AND ((f.alert_type='change' AND ABS((p.price-f.baseline_price)/NULLIF(f.baseline_price,0)*100)>=f.threshold_percent) OR (f.alert_type='below_price' AND p.price<=f.target_price)) ORDER BY f.created_at DESC`,[req.user.id]);res.json({methodology:"Change alerts compare today’s Elohim price with your follow price. Target alerts appear only when Elohim’s selling price reaches your chosen amount.",alerts:result.rows});}catch(err){console.error("PRICE ALERTS ERROR:",err);res.status(500).json({error:"Failed to load price alerts"});} });
