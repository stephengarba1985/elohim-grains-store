...
export default function TrackOrder() {
  ...
  if (!data) {
    return (
      <main className="min-h-screen bg-slate-50 p-4 md:p-6" aria-label="Loading delivery tracking"><div className="mx-auto max-w-5xl space-y-6"><div className="h-44 animate-pulse rounded-2xl bg-slate-900" /><div className="grid gap-4 sm:grid-cols-2 md:grid-cols-4">{Array.from({ length: 4 }).map((_, index) => <div key={index} className="h-24 animate-pulse rounded-xl bg-slate-200" />)}</div><div className="h-72 animate-pulse rounded-2xl bg-white" /></div></main>
    );
  }
