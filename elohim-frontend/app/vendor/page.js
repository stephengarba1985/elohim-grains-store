...
export default function VendorDashboard() {
  ...
  return <main className="min-h-screen bg-slate-50 p-6"><div className="mx-auto max-w-6xl">
    ...
    {tab === "Orders" && <section className="mt-6 rounded-xl bg-white p-5 shadow-sm"><h2 className="text-xl font-black">Order fulfilment</h2><p className="mt-1 text-sm text-slate-500">Accept → Preparing → Ready for collection/delivery. Meet each deadline to protect your vendor standing.</p>{data.orders.map((o) => { const action = next[o.vendor_fulfilment_status]; const model = o.fulfilment_model === "fulfilled_by_elohim" ? "Fulfilled by Elohim" : "Vendor prepared, Elohim delivery"; return <div key={o.id} className="mt-4 rounded border border-slate-200 p-4"><div className="flex flex-wrap justify-between gap-3"><div><p className="font-bold">{o.order_reference || `Vendor order #${o.id}`}</p><p>{o.product_name} × {o.quantity}</p><p className="mt-1 text-sm text-slate-500">{model}</p><p className="mt-1 text-sm text-slate-500">Fulfilment deadline: {date(o.fulfilment_deadline)}</p><p className="mt-1 text-sm capitalize text-slate-500">Status: {o.vendor_fulfilment_status || "new"} · Payment: {o.payment_status}</p></div><div className="text-right"><p className="font-bold text-emerald-700">{money(o.total_amount)}</p>{action && <button onClick={() => advance(o, action[0])} className="mt-3 rounded bg-emerald-700 px-3 py-2 text-xs font-bold text-white">{action[1]}</button>}</div></div></div>; })}</section>}
